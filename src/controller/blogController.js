...
const createBlog = async function (req, res) {
  try {
    let data = req.body;
    let {title, body, authorId, tags, category, subcategory, isPublished, ...rest } = data

    if(checkInput(rest)) return res.status(400).send({status:false, message: "You can enter only title, body, authorId, tags, category, subcategory, isPublished"})

    if (!title) return res.status(400).send({ status: false, message: "Please enter title" });
    if (!body) return res.status(400).send({ status: false, message: "Please enter body" });
    ...
    if(!mongoose.isValidObjectId(id)) return res.status(400).send({status:false, message:"Please enter valid author id"})
...
const getBlog = async function (req, res) {
  try {
    ...
    if(checkInput(rest)) return res.status(400).send({status: false, message:"Only acceptable authorId, tags, subcategory, category "})
